...
// ─────────────────────────────────────────────────────────────────────────
// /14display — 14" display variant of the main dashboard.
//
// This is a deliberate fork of app/page.tsx, not a refactor of it. The
// production dashboard at / is design-locked and must keep rendering exactly
// as it does today, so new layout work lands here and gets validated live in
// the same container before anything is merged back into /.
//
// Differences from app/page.tsx:
//   1. The away/driving map uses CARTO "Dark Matter" tiles (MapTile14)
//      instead of standard OSM raster.
//   2. Two extra tiles along the bottom: a 7-day charge sparkline and a
//      fuel-savings comparison, both fed by /api/energy.
...
// Leaflet touches window/document at import time -- ssr:false, same as the
// main route's MapTile (see components/MapTile14.tsx).
const MapTile14 = dynamic(() => import('@/components/MapTile14'), { ssr: false });

const REFRESH_MS = 30_000;
// /api/energy only changes when a charging session ends, and its gas price is
// a once-a-day EIA figure -- no reason to pull it on the 30s vehicle cadence.
...
// ── Bottom-row tiles (/14display only) ───────────────────────────────────────

// Per-vehicle series colors. Mirrors the intent of CircuitPanel's per-side
// palette (one cool grey, one cool blue) but keyed on the vehicle rather than
// the connector side, so the sparkline stays stable if the sides get swapped.
const VEH_COLOR: Record<'rivian' | 'tesla', string> = { rivian: '#9aa5b1', tesla: '#5b8def' };

const TILE_STYLE: React.CSSProperties = {
  flex: 1, minWidth: 0, background: '#12181e', border: '1px solid rgba(255,255,255,0.06)',
  borderRadius: 20, padding: '11px 18px', display: 'flex', flexDirection: 'column', gap: 8,
};
const TILE_LABEL: React.CSSProperties = {
  fontFamily: "'JetBrains Mono',monospace", fontSize: 9.5, letterSpacing: '0.14em', color: '#7d8893',
};

// 7-day daily energy per vehicle. Hand-rolled SVG, matching the precedent set
// by the charge dial in VehicleCard.tsx -- no charting dependency.
function ChargeHistoryTile({ energy }: { energy: EnergySummary | null }) {
  const W = 470, H = 52, PAD = 1;
  const days = energy?.days ?? [];
  const series = energy?.vehicles ?? [];
  const peak = Math.max(0.1, ...series.flatMap(v => v.dailyKwh));
  const weekTotal = series.reduce((sum, v) => sum + v.dailyKwh.reduce((a, b) => a + b, 0), 0);

  const groupW = days.length ? W / days.length : W;
  const barW = Math.max(3, (groupW - 8) / Math.max(1, series.length) - PAD);

  return (
    <div style={TILE_STYLE}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 10 }}>
        <span style={TILE_LABEL}>CHARGE HISTORY · 7 DAYS</span>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, color: '#a4afba' }}>
          {weekTotal.toFixed(1)} kWh total
        </span>
      </div>

      {days.length === 0 ? (
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: '#5e6873' }}>NO SESSIONS RECORDED YET</span>
      ) : (
        <>
          <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: 'block' }}>
            {days.map((_, di) => series.map((v, si) => {
              const kwh = v.dailyKwh[di] ?? 0;
              // Floor at 1px so a day with a tiny-but-real charge still reads
              // as present rather than vanishing into the baseline.
              const h = kwh > 0 ? Math.max(1, (kwh / peak) * (H - 2)) : 0;
              const x = di * groupW + 4 + si * (barW + PAD);
              return h > 0 ? (
                <rect key={`${di}-${si}`} x={x} y={H - h} width={barW} height={h} rx={1.5} fill={VEH_COLOR[v.id]} />
              ) : null;
            }))}
            <line x1={0} y1={H - 0.5} x2={W} y2={H - 0.5} stroke="rgba(255,255,255,0.07)" strokeWidth={1} />
          </svg>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
            <div style={{ display: 'flex', gap: 12 }}>
              {series.map(v => (
                <span key={v.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: '#7d8893' }}>
                  <span style={{ width: 7, height: 7, borderRadius: 2, background: VEH_COLOR[v.id] }} />
                  {v.name}
                </span>
              ))}
            </div>
            <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: '#5e6873' }}>
              {days[0]?.slice(5)} → {days[days.length - 1]?.slice(5)} · PEAK {peak.toFixed(1)} kWh/DAY
            </span>
          </div>
        </>
      )}
...
// Actual TOU electricity cost vs. what the same miles would have cost in a
// comparable gas car. Rate table + comparables live in lib/rates.ts.
function FuelSavingsTile({ energy }: { energy: EnergySummary | null }) {
  const saved = energy?.monthSavedUsd ?? null;
  const last = energy?.lastSession ?? null;
  const usd = (n: number) => '$' + n.toFixed(2);
  const savedColor = saved === null ? '#7d8893' : saved >= 0 ? ACCENT : '#e2685f';

  return (
    <div style={TILE_STYLE}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 10 }}>
        <span style={TILE_LABEL}>FUEL SAVINGS · THIS MONTH</span>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: '#5e6873' }}>
          {/* The week the price is from matters: with no EIA key (or a failing
              fetch) the cached figure is served indefinitely, so show its
              period rather than passing off a stale price as current. */}
          {energy?.gasPriceUsdPerGallon != null
            ? `GAS ${usd(energy.gasPriceUsdPerGallon)}/gal · ${energy.gasPriceLabel}`
              + (energy.gasPricePeriod ? ` · wk ${energy.gasPricePeriod}` : '')
            : 'GAS PRICE UNAVAILABLE · SET EIA KEY IN SETTINGS'}
        </span>
      </div>

      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 14 }}>
        <span style={{ fontSize: 30, fontWeight: 600, lineHeight: 0.9, letterSpacing: '-0.02em', color: savedColor }}>
          {saved === null ? '—' : usd(saved)}
        </span>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, color: '#a4afba', paddingBottom: 2 }}>
          {energy ? `${energy.monthKwh.toFixed(1)} kWh · ${usd(energy.monthElectricityUsd)} electric` : '—'}
          {energy?.monthGasEquivalentUsd != null ? ` vs ${usd(energy.monthGasEquivalentUsd)} gas` : ''}
        </span>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginTop: 'auto', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 7 }}>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: '#7d8893' }}>
          {last
            ? `LAST · ${last.vehicleName} · ${last.kwh.toFixed(1)} kWh · ${usd(last.electricityUsd)}`
              + (last.gasEquivalentUsd != null ? ` vs ${usd(last.gasEquivalentUsd)} gas → ${usd(last.savedUsd ?? 0)} saved` : '')
            : 'NO COMPLETED SESSIONS YET'}
        </span>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 8, letterSpacing: '0.08em', color: '#5e6873' }}>
          EST · {energy?.rateLabel ?? '—'}
          {energy?.vehicles.length ? ' · vs ' + energy.vehicles.map(v => `${v.comparable} (${v.mpg} mpg)`).join(' / ') : ''}
        </span>
      </div>
    </div>
  );
...
function DashboardInner() {
  ...
  // Bottom-row tiles. Failures leave `energy` null, which both tiles render
  // as an explicit empty state rather than blanking the row.
  useEffect(() => {
    const load = () => fetch('/api/energy', { cache: 'no-store' })
      .then(r => r.ok ? r.json() as Promise<EnergySummary> : null)
      .then(e => { if (e) setEnergy(e); })
      .catch(() => null);
    load();
    const t = setInterval(load, ENERGY_REFRESH_MS);
  ...
  return (
    <div style={{ position: 'relative', width: 1180, height: 820, overflow: 'hidden', background: 'radial-gradient(1000px 600px at 78% -16%, #1a2530 0%, #0e1216 56%)', color: '#e8edf2', fontFamily: "'Space Grotesk',sans-serif", padding: '18px 28px', display: 'flex', flexDirection: 'column', gap: 7 }}>
      ...
      {/* ── Bottom row: charge history + fuel savings (/14display only) ── */}
      <div style={{ display: 'flex', gap: 16, flex: 'none' }}>
        <ChargeHistoryTile energy={energy} />
        <FuelSavingsTile energy={energy} />
      </div>
