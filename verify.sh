#!/usr/bin/env bash
# verify for: ev-circuit-charging-truth
...
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'lib/circuitStatus.ts' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: lib/circuitStatus.ts parses"
  ...
  echo "  FAIL: lib/circuitStatus.ts does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
# but only FAIL when the error is in the file the model edited (lib/circuitStatus.ts).
...
elif grep -qE 'lib/circuitStatus\.ts[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in lib/circuitStatus.ts"; grep -E 'lib/circuitStatus\.ts[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE lib/circuitStatus.ts (not this task's) -- passing type gate"
fi
