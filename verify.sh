#!/usr/bin/env bash
# verify for: ev-influx-write-request
...
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'server/telemetry-influx.js' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: server/telemetry-influx.js parses"
  ...
  echo "  FAIL: server/telemetry-influx.js does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
# but only FAIL when the error is in the file the model edited (server/telemetry-influx.js).
...
elif grep -qE 'server/telemetry\-influx\.js[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in server/telemetry-influx.js"; grep -E 'server/telemetry\-influx\.js[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE server/telemetry-influx.js (not this task's) -- passing type gate"
fi
