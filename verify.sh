...
if "$NODE" '/Users/penn/bin/ts-mutator/ts-parse.mjs' 'server/influx-line-protocol.js' 2>/tmp/_verify_parse.$$.log; then
  echo "  ok: server/influx-line-protocol.js parses"
  ...
  echo "  FAIL: server/influx-line-protocol.js does not parse"; head -5 /tmp/_verify_parse.$$.log; fails=$((fails+1))
fi
...
# but only FAIL when the error is in the file the model edited (server/influx-line-protocol.js).
...
elif grep -qE 'server/influx\-line\-protocol\.js[(:]' /tmp/_verify_tsc.$$.log; then
  echo "  FAIL: tsc --noEmit reports errors in server/influx-line-protocol.js"; grep -E 'server/influx\-line\-protocol\.js[(:]' /tmp/_verify_tsc.$$.log | head -15; fails=$((fails+1))
else
  echo "  WARN: tsc --noEmit has pre-existing errors OUTSIDE server/influx-line-protocol.js (not this task's) -- passing type gate"
fi
